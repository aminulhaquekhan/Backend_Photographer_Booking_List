...
<div class="container mx-auto py-8">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div class="bg-white shadow rounded-lg flex items-center p-6">
            ...
            <div>
                <h5 class="text-gray-500 mb-1 font-semibold">Total Photographers</h5>
                <h2 class="text-2xl font-bold">{{ $photographers->count() }}</h2>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="bg-green-500 text-white rounded-t-lg px-6 py-3 font-bold flex justify-between items-center">
                <span>Recent Photographers</span>
                @if($photographers->count() > 10)
                <a href="{{ route('photographers.index') }}" class="text-sm text-white hover:underline">View All</a>
                @endif
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($photographers->take(10) as $photographer)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $loop->iteration }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $photographer->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $photographer->speciality ?? 'N/A' }}</td>
                        </tr>
                        @empty
                        <tr>
